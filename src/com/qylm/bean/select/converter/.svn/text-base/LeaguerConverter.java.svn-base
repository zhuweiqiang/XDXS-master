 ...
 * @author smj
 */
@FacesConverter("leaguerConverter")
public class LeaguerConverter implements Converter {

	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				LeaguerService service = ApplicationContextHelper.getBean("leaguerService");
				return service.getById(Integer.parseInt(value));
			} catch (NumberFormatException e) {
				throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "选择错误", "没有此卡项级别！"));
			}
		} else {
			return null;
		}
