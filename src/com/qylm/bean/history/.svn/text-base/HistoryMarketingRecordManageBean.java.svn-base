...
import com.qylm.bean.returner.history.HistoryMarketingRecordManageReturner;
...
@ManagedBean
@RequestScoped
public class HistoryMarketingRecordManageBean extends BasePagingBean {

	/**
	 * serialVersionUID
	 */
	private static final long serialVersionUID = 4563621266855000178L;

	/**
	 * LOG
	 */
	private static final Log LOG = LogFactory.getLog(HistoryMarketingRecordManageBean.class);
	
	/**
	 * 保存事件管理画面需要保存的值
	 */
	private HistoryMarketingRecordManageDto historyMarketingRecordManageDto = new HistoryMarketingRecordManageDto();
	...
	public String getAll() {
		Tool.sendLog(LOG, userBean, "按下【功能菜单_客户产品记录菜单】");
		fetchData(0, true);
		return Navigation.HISTORY_MARKETING_RECORD_MANAGE;
	}
	
	/**
	 * 此方法绑定于客户产品记录管理画面的新建按钮 
	 * 实现功能：跳转至客户产品记录登陆画面，新建一家事件
	 * @return 客户产品记录登陆画面
	 */
	public String newMarketingRecord() {
		Tool.sendLog(LOG, userBean, "按下【客户产品记录管理画面_新建按钮】");
		return Tool.getBackBean(HistoryMarketingRecordCreateBean.class).newCreate(new HistoryMarketingRecordManageReturner(historyMarketingRecordManageDto, currentPage));
	}

	/**
	 * 此方法绑定于客户产品记录管理画面的修改按钮 
	 * 实现功能：根据修改的对象，跳转至客户产品记录登陆画面
	 * @return 客户产品记录登陆画面
	 */
	public String updateMarketingRecord(MarketingRecord transferMarketingRecord) {
		Tool.sendLog(LOG, userBean, "按下【客户产品记录管理画面_修改按钮】");
		return Tool.getBackBean(HistoryMarketingRecordCreateBean.class).updateDetail(new HistoryMarketingRecordManageReturner(historyMarketingRecordManageDto, currentPage), transferMarketingRecord);
	}
	
	/**
	 * 此方法绑定于客户产品记录管理画面的检索按钮 
	 * 实现功能：根据检索条件，检索出事件
	 * @return 画面不跳转
	 */
	public void selectMarketingRecord() {
		Tool.sendLog(LOG, userBean, "按下【客户产品记录管理画面_检索按钮】");
		fetchData(0, true);
	}
	
	/**
	 * 绑定于客户产品记录管理画面的全选删除按钮
	 * @param event
	 */
	public void deleteMul(ActionEvent event) {
		Tool.sendLog(LOG, userBean, "按下【客户产品记录管理画面_批量删除按钮】");
		try {
	...
	public String viewDetail(MarketingRecord transferMarketingRecord) {
		Tool.sendLog(LOG, userBean, "按下【客户产品记录管理画面的_视图按钮】");
		if (transferMarketingRecord == null) {
			transferMarketingRecord = marketingRecordService.getById(Integer.valueOf(super.modelId));
		}
		return Tool.getBackBean(HistoryMarketingRecordViewBean.class).viewDetail(transferMarketingRecord);
	}
	...
	protected void fetchData(int start, boolean needInit) {
		...
		String name = historyMarketingRecordManageDto.getName();
		Date beginDate = historyMarketingRecordManageDto.getBeginDate();
		Date endDate = historyMarketingRecordManageDto.getEndDate();
	...
	/**
	 * get historyMarketingRecordManageDto
	 * @return the historyMarketingRecordManageDto
	 */
	public HistoryMarketingRecordManageDto getHistoryMarketingRecordManageDto() {
		return historyMarketingRecordManageDto;
	}

	/**
	 * set historyMarketingRecordManageDto
	 * @param historyMarketingRecordManageDto the historyMarketingRecordManageDto to set
	 */
	public void setHistoryMarketingRecordManageDto(HistoryMarketingRecordManageDto historyMarketingRecordManageDto) {
		this.historyMarketingRecordManageDto = historyMarketingRecordManageDto;
	}
