...
@ManagedBean
@RequestScoped
public class HistoryTemporaryManageBean extends BasePagingBean {

	/**
	 * serialVersionUID
	 */
	private static final long serialVersionUID = -5530789890144907898L;

	/**
	 * LOG
	 */
	private static final Log LOG = LogFactory.getLog(HistoryTemporaryManageBean.class);
	
	/**
	 * 保存事件管理画面需要保存的值
	 */
	private HistoryTemporaryManageDto historyTemporaryManageDto = new HistoryTemporaryManageDto();
	...
	public String getAll() {
		Tool.sendLog(LOG, userBean, "按下【功能菜单_客户套餐记录菜单】");
		fetchData(0, true);
		return Navigation.HISTORY_TEMPORARY_MANAGE;
	}
	
	/**
	 * 此方法绑定于客户套餐记录管理画面的新建按钮 
	 * 实现功能：跳转至客户套餐记录登陆画面，新建一家事件
	 * @return 客户套餐记录登陆画面
	 */
	public String newMealBuyRecord() {
		Tool.sendLog(LOG, userBean, "按下【客户套餐记录管理画面_新建按钮】");
		return Tool.getBackBean(HistoryTemporaryCreateBean.class).newCreate(new HistoryTemporaryManageReturner(historyTemporaryManageDto, currentPage));
	}

	/**
	 * 此方法绑定于客户套餐记录管理画面的修改按钮 
	 * 实现功能：根据修改的对象，跳转至客户套餐记录登陆画面
	 * @return 客户套餐记录登陆画面
	 */
	public String updateMealBuyRecord(MealBuyRecord transferMealBuyRecord) {
		Tool.sendLog(LOG, userBean, "按下【客户套餐记录管理画面_修改按钮】");
		return Tool.getBackBean(HistoryTemporaryCreateBean.class).updateDetail(new HistoryTemporaryManageReturner(historyTemporaryManageDto, currentPage), transferMealBuyRecord);
	}
	
	/**
	 * 此方法绑定于客户套餐记录管理画面的检索按钮 
	 * 实现功能：根据检索条件，检索出事件
	 * @return 画面不跳转
	 */
	public void selectMealBuyRecord() {
		Tool.sendLog(LOG, userBean, "按下【客户套餐记录管理画面_检索按钮】");
		fetchData(0, true);
	}
	
	/**
	 * 绑定于客户套餐记录管理画面的全选删除按钮
	 * @param event
	 */
	public void deleteMul(ActionEvent event) {
		Tool.sendLog(LOG, userBean, "按下【客户套餐记录管理画面_批量删除按钮】");
	...
	public String viewDetail(MealBuyRecord transferMealBuyRecord) {
		Tool.sendLog(LOG, userBean, "按下【客户套餐记录管理画面的_视图按钮】");
		if (transferMealBuyRecord == null) {
			transferMealBuyRecord = mealBuyRecordService.getById(Integer.valueOf(super.modelId));
		}
		return Tool.getBackBean(HistoryTemporaryViewBean.class).viewDetail(transferMealBuyRecord);
	}
	...
	protected void fetchData(int start, boolean needInit) {
		...
		String name = historyTemporaryManageDto.getName();
		String leaguerNumber = historyTemporaryManageDto.getLeaguerNumber();
		Date beginDate = historyTemporaryManageDto.getBeginDate();
		Date endDate = historyTemporaryManageDto.getEndDate();
	...
	/**
	 * get historyTemporaryManageDto
	 * @return the historyTemporaryManageDto
	 */
	public HistoryTemporaryManageDto getHistoryTemporaryManageDto() {
		return historyTemporaryManageDto;
	}

	/**
	 * set historyTemporaryManageDto
	 * @param historyTemporaryManageDto the historyTemporaryManageDto to set
	 */
	public void setHistoryTemporaryManageDto(HistoryTemporaryManageDto historyTemporaryManageDto) {
		this.historyTemporaryManageDto = historyTemporaryManageDto;
	}
