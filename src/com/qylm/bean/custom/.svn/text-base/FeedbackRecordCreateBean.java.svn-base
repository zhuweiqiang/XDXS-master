...
import com.qylm.dto.custom.FeedbackRecordCreateDto;
import com.qylm.dxo.FeedbackRecordCreateDxo;
...
@ManagedBean
@RequestScoped
public class FeedbackRecordCreateBean implements Serializable, CreateBean<FeedbackRecord> {

	/**
	 * serialVersionUID
	 */
	private static final long serialVersionUID = -2252856209135338942L;

	/**
	 * LOG
	 */
	private static final Log LOG = LogFactory.getLog(FeedbackRecordCreateBean.class);

	/**
	 * 存放客户反馈记录登陆画面需要保存的值
	 */
	private FeedbackRecordCreateDto feedbackRecordCreateDto = new FeedbackRecordCreateDto();
	...
	public String newFeedbackRecord() {
		...
		feedbackRecordCreateDto.setCustomInfo(null);
		feedbackRecordCreateDto.setDate(DateUtil.getNowyyyymmdd());
		feedbackRecordCreateDto.setRemark(null);
		feedbackRecordCreateDto.setMarketingProject(null);
		feedbackRecordCreateDto.setProductStock(null);
		feedbackRecordCreateDto.setPersonnelInfo(null);
		feedbackRecordCreateDto.setCreater(null);
		feedbackRecordCreateDto.setBelongingUser(null);
		feedbackRecordCreateDto.setTransferFeedbackRecord(null);
		return Navigation.FEEDBACK_RECORD_CREATE;
	}
	...
	public String back() {
		...
		return feedbackRecordCreateDto.getReturner().returnOnly();
	}
	...
	public void saveFeedbackRecord() {
		...
		FeedbackRecord transferFeedbackRecord = feedbackRecordCreateDto.getTransferFeedbackRecord();
		if (transferFeedbackRecord == null) {
			transferFeedbackRecord = new FeedbackRecord();
			feedbackRecordCreateDto.setCreater(userBean.getUser());
			feedbackRecordCreateDto.setBelongingUser(MothedUtil.getBelongingUser(userBean.getUser()));
			...
			feedbackRecordCreateDto.setTransferFeedbackRecord(transferFeedbackRecord);
	...
	private void create(FeedbackRecord transferFeedbackRecord) {
		FeedbackRecordCreateDxo.dtoToEntity(feedbackRecordCreateDto, transferFeedbackRecord);
	}

	public String newCreate(Returner<?, ?, ?> returner) {
		feedbackRecordCreateDto.setDate(DateUtil.getNowyyyymmddhhmmss());
		feedbackRecordCreateDto.setCreater(userBean.getUser());
		feedbackRecordCreateDto.setReturner(returner);
		return Navigation.FEEDBACK_RECORD_CREATE;
	}

	public String updateDetail(Returner<?, ?, ?> returner, FeedbackRecord feedbackRecord) {
		feedbackRecordCreateDto.setReturner(returner);
		feedbackRecordCreateDto.setTransferFeedbackRecord(feedbackRecord);
		FeedbackRecordCreateDxo.entityToDto(feedbackRecord, feedbackRecordCreateDto);
		return Navigation.FEEDBACK_RECORD_CREATE;
	}
	...
	private void select() {
		CustomInfo customInfo = feedbackRecordCreateDto.getCustomInfo();
		if (customInfo == null) {
		...
		feedbackRecordCreateDto.setProductStockList(productStockList);
	}
	...
	public void findProductStocks(ProductStock productStock) {
		...
		feedbackRecordCreateDto.setProductStock(productStock);
	}
	...
	public void findMarketingProject() {
		...
		feedbackRecordCreateDto.setMarketingProjectList(marketingProjectList);
	}
	...
	public void findMarketingProjects(MarketingProject marketingProject) {
		Tool.sendLog(LOG, userBean, "按下【客户反馈登陆画面_选择按钮】");
		feedbackRecordCreateDto.setMarketingProject(marketingProject);
	}
	...
	/**
	 * get feedbackRecordCreateDto
	 * @return the feedbackRecordCreateDto
	 */
	public FeedbackRecordCreateDto getFeedbackRecordCreateDto() {
		return feedbackRecordCreateDto;
	}

	/**
	 * set feedbackRecordCreateDto
	 * @param feedbackRecordCreateDto the feedbackRecordCreateDto to set
	 */
	public void setFeedbackRecordCreateDto(FeedbackRecordCreateDto feedbackRecordCreateDto) {
		this.feedbackRecordCreateDto = feedbackRecordCreateDto;
	}
